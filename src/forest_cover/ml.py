...
from .loadable import LoadableLogit, LoadableTree, LoadableForest, \
    LoadablekNN, LoadableLogitHyperSearch, LoadableTreeHyperSearch, \
    LoadableForestHyperSearch, LoadableKnnHyperSearch
...
class MLApp(cmd2.Cmd):
    ...
    def __init__(self) -> None:
        ...
        self.intro = cmd2.style('Начните работу с выбора алгоритма '
    ...
    setmodel_parser.add_argument('algorythm', type=str,
    ...
    def do_setmodel(self, ns: argparse.Namespace) -> None:
        ...
        try:
            self.register_command_set(getattr(self,
                                              f'_{ns.algorythm}'))
            self.register_command_set(getattr(self,
                                              f'_hypersearch{ns.algorythm}'))
            self.poutput(f'Выбрана модель: {ns.algorythm}. Настройте '
    ...
    setpath_subparsers = setpath_parser.add_subparsers(title='подкоманды',
                                                       help='справка по '
                                                            'подкомандам:')

    parser_load = setpath_subparsers.add_parser('load',
                                                help='задать путь к '
                                                     'анализируемому датасету')
    parser_load.add_argument('input_file',
                             type=str,
                             help='csv-файл с анализируемыми данными')
    parser_export = setpath_subparsers.add_parser('export',
                                                  help='задать путь для '
                                                       'сохранения csv-файла '
                                                       'с предиктом')
    parser_export.add_argument('output_file', type=str,
                               help='csv-файл, который будет создан '
                                    'по результам работы предикта')
    parser_dump = setpath_subparsers.add_parser('dump',
                                                help='задать путь для сохране'
                                                     'ния параметров модели')
    parser_dump.add_argument('dump_file',
                             type=str,
                             help='joblib-файл, в который будут '
                                  'сохраняться параметры модели')
    ...
    def setpath_load(self, args: argparse.Namespace) -> None:
        ...
        if check_file_exists(filepath):
            if check_extension(filepath, 'csv'):
                ...
                raise ValueError('Указан путь к файлу '
                                 'с форматом, отличным от csv')
        else:
            raise FileNotFoundError(f'Файл по указанному пути '
                                    f'({filepath}) не найден')
    ...
    def setpath_export(self, args: argparse.Namespace) -> None:
        ...
        if check_dir_exists(filepath):
            if check_extension(filepath, 'csv'):
                ...
                self.poutput(f'Установлен путь '
                             f'для выгрузки предикта: {filepath}')
            else:
                raise ValueError('Указан путь к файлу '
                                 'с форматом, отличным от csv')
        else:
            raise FileNotFoundError(f'Указанная папка для размещения '
                                    f'файла не найдена ({filepath})')
    ...
    def setpath_dump(self, args: argparse.Namespace) -> None:
        ...
        if check_dir_exists(filepath):
            if check_extension(filepath, 'joblib'):
                ...
                self.poutput(f'Установлен путь для выгрузки данных '
                             f'модели: {filepath}')
            else:
                raise ValueError('Указан путь к файлу с '
                                 'форматом, отличным от joblib')
        else:
            raise FileNotFoundError(f'Указанная папка для размещения '
                                    f'файла не найдена ({filepath})')
    ...
    def do_paths(self, args: argparse.Namespace) -> None:
        if args.reset:
            ...
            self.poutput(f"Путь к датасету:"
    ...
    scaler_parser.add_argument('scaler',
                               type=str,
    ...
    def do_scaler(self, args: argparse.Namespace) -> None:
        ...
        if args.scaler == 'none':
            ...
            self.poutput(f'Установлен алгоритм масштабирования '
                         f'данных: {args.scaler}')

    # DIMREDUCT

    dimreduct_parser = cmd2.Cmd2ArgumentParser()
    dimreduct_parser.add_argument('dimreduct',
                                  type=str,
    ...
    def do_dimreduct(self, args: argparse.Namespace) -> None:
        ...
        if args.dimreduct == 'none':
            ...
            self.poutput(f'Установлен алгоритм уменьшения размерности '
                         f'данных: {args.dimreduct}')

    # FEATENG

    feateng_parser = cmd2.Cmd2ArgumentParser()
    feateng_parser.add_argument('feateng',
                                type=str,
                                choices=['none', 'auto'],
        ...
        if args.feateng == 'none':
            ...
            if not self.data[0] or self.config['feateng'] == 'none':
                ...
                if hashlib.sha1(pd.util.hash_pandas_object(
                        self.data[0]).values).hexdigest() != FOREST_COVER_HASH:
                    self.poutput('Эта опция доступна только для датасета '
                                 'Forest Cover Type Prediction')
                else:
                    self.poutput('Проводим feature engineering...')
                    self.data = make_new_features(self.data[0]), self.data[1]
                    self.poutput('Успешно! Теперь будет использоваться '
